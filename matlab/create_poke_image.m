function outputImage = create_poke_image(imagePath, databasePath)
...
if size(inputImage, 1) < 50 || size(inputImage, 2) < 50
    ...
    inputImage = imresize(inputImage, 1.5);
...
fileList = dir(fullfile(databasePath, '*.jpg'));
...
% All the average colors from the data
dataBase_avgColors = zeros(numel(fileList), 3);  % Matrix to store average colors (R, G, B)

for i = 1:numel(fileList)
    filename = fullfile(databasePath, fileList(i).name);
